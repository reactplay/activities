    ...
    return (
        <section
            ...
            className={`flex flex-col items-center justify-center w-full mx-auto px-4 ${
                events && 'bg-gray-50'
            } ${twitterSpaces && 'bg-cyan-400/10'} ${reactPlayLive && 'bg-slate-900'}`}
        >
            <h1 className={`text-5xl py-16 font-sans ${reactPlayLive && 'text-white'}`}>
                {!title.split(' ')[1] ? (
            ...
            <div className="flex flex-col lg:grid lg:grid-cols-3 justify-center items-center gap-10 lg:gap-20">
                ...
                {twitterSpaces?.map((el) => (
                    <div key={el.id} className="bg-slate-900 text-white w-[350px] p-4 rounded-lg">
                        {/* Host */}
