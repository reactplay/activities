...
//   console.log('map',map,'value', value);
  return (
            ...
            <li key={oi} className={`${styles["tl-item"]}`}>
              <div
                className={`${styles["item-title"]} ${
                  current_value ? "text-white" : "text-grey-200"
                }`}
              >
                {o}
              </div>
              <div className={`text-brand-muted  ${styles["item-detail"]} `}>
                {" "}
                {current_value
                  ? moment(current_value.date).format("DD-MM-yyyy")
                  : "Pending"}
              </div>
            </li>
          );
